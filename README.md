...
Note: We defaulted the setup on this repository to validate each implementation against the serial one. Because of the need for _random_ initialization in the k clusters, and the variable number of epochs and clusters, we run the serial and then execute whatever function you are using. For example, when running the single GPU implementation, we execute the serial implementation followed by the GPU implementation, so that the data can be validated.
...
Unfortunatly, both tables show that the parallel implementation is not scalable. However, the parallel implementation is still 1.5-2.5x faster than the serial implementation.
